...
export default function Dashboard() {
  ...
  const [selectedMembers, setSelectedMembers] = useState([]); // Users selected for new project
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {projects.map((project) => (
          ...
          >
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-purple-500 transition-colors">{project.name}</h3>
            ...
            <div className="mt-4 flex items-center gap-2">
                ...
                <div className="flex -space-x-2">
                    {project.members?.slice(0, 4).map(m => (
                        <div key={m.id} className="w-6 h-6 rounded-full bg-purple-100 dark:bg-purple-900 border-2 border-white dark:border-gray-800 flex items-center justify-center text-[10px] text-purple-700 dark:text-purple-300 font-bold" title={m.email}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                  ...
                  <div className="border dark:border-gray-600 rounded-md p-2 max-h-40 overflow-y-auto bg-gray-50 dark:bg-gray-700/50">
                      {users.length === 0 ? (
                          <p className="text-xs text-gray-500 text-center p-2">No other users found.</p>
                      ) : (
                          users.map(u => (
                              <div key={u.id} className="flex items-center gap-2 mb-2 last:mb-0">
                                  <input 
                                    type="checkbox" 
                                    id={`user-${u.id}`}
                                  ...
                                  />
                                  <label htmlFor={`user-${u.id}`} className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
                                      {u.email}
                                  </label>
                              </div>
                          ))
